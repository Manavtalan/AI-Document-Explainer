...
const SUGGESTIONS = [
  ...
  "Should I negotiate the non-compete?",
  "Explain the clawback terms",
];
...
const ChatPanel = () => {
  ...
  const { chatMessages, addChatMessage, extractedText, analysis } = useOfferLetterStore();
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("offer-letter-qa", {
        body: {
  ...
  if (!isOpen) {
    return (
      ...
      >
        ...
        <span className="text-sm font-medium hidden sm:inline">Ask about your offer</span>
      </button>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {chatMessages.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">
            Ask any question about your offer letter analysis.
          </p>
        )}
