...
import { OfferLetterSection } from "@/hooks/useOfferLetterStore";
...
const SectionCard = ({ section }: { section: OfferLetterSection }) => {
  ...
  return (
    <div
      className={`rounded-xl border bg-card overflow-hidden border-l-4 ${config.border}`}
    >
      <button
      ...
      {expanded && (
        <div className="px-4 md:px-5 pb-5 space-y-4 animate-fade-in">
          {/* Original text */}
          {section.original_text && section.original_text !== "Not found in offer letter" && (
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-1.5">What Your Offer Says:</p>
...
const SectionBreakdown = ({ sections }: Props) => {
