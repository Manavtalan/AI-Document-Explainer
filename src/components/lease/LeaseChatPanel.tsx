...
const SUGGESTIONS = [
  ...
  "Can I sublet to someone else?",
  "What happens if I pay rent late?",
];

const MAX_MESSAGES = 20;

const LeaseChatPanel = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const { chatMessages, addChatMessage, extractedText, analysis } = useLeaseStore();
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("lease-qa", {
        body: {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-full bg-teal-600 text-white shadow-lg hover:bg-teal-700 transition-colors"
      >
        <MessageCircle className="w-5 h-5" />
        <span className="text-sm font-medium hidden sm:inline">Ask about your lease</span>
      </button>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {chatMessages.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">
            Ask any question about your lease agreement analysis.
          </p>
        )}
