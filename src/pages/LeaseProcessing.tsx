...
const LOADING_MESSAGES = [
  ...
  "Analyzing rental terms and conditions…",
  "Checking for red flags and hidden fees…",
  "Reviewing your tenant rights…",
...
const LeaseProcessing = () => {
  ...
  const activeFile = leaseFile || docStoreFile;
  ...
  useEffect(() => {
    ...
    const process = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setProcessingError("We couldn't analyze this lease right now. Please try again.");
          setError("Analysis failed");
          return;
        }

        if (!data?.success) {
          setProcessingError(data?.error || "We couldn't analyze this lease right now. Please try again.");
    ...
    process();
  }, [activeFile, leaseFile, docStoreFile, navigate, setFile, setIsAnalyzing, setError, setExtractedText, setAnalysis, navigateWithMinTime]);
      ...
      <main className="flex flex-col items-center justify-center min-h-[calc(100vh-64px)] px-4">
        {processingError ? (
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground">This usually takes 15–20 seconds for a full lease</p>
            {(fileName || activeFile?.name) && (
              <p className="text-sm text-muted-foreground mt-4 truncate max-w-xs mx-auto">
                {fileName || activeFile?.name}
              </p>
            )}
            <button onClick={handleTryAgain} className="mt-6 text-sm text-muted-foreground hover:text-foreground transition-colors">
              Cancel
            </button>
          </div>
        )}
...
export default LeaseProcessing;
